# Building a classifier based on Gaussian Mixture Models(GMMs)
...
indices = skf.split(iris.data, iris.target)
...
for i, color in enumerate(colors):
    ...
    #normalize thefirst eigenvectors
    norm_vec = eigenvectors[0]/np.linalg.norm(eigenvectors[0])
    # Extract the angle of tilt
    angle = np.arctan2(norm_vec[1], norm_vec[0])
    angle = (180 * angle/np.pi)
    # Scalling factor to magnify the ellipses
    scaling_factor = 8
...
for i, color in enumerate(colors):
    ...
    plt.scatter(cur_data[:,0], cur_data[:,1], marker='o', facecolors='none',
                edgecolors='black', s=40, label=iris.target_names[i])
    test_data = X_test[y_test==i]
    plt.scatter(test_data[:,0], test_data[:, 1], marker='s', facecolors='black',
                edgecolors='black', s=40, label=iris.target_names[i])
